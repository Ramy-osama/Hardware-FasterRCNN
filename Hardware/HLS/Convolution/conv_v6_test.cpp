...
typedef ap_fixed<word_size,int_width> fp;
...
void convo ( fp input_image[image_channels][image_size][image_size],
			fp kernel[kernel_filters][kernel_channels][kernel_size][kernel_size],
			fp bias[kernel_biases],
			fp conv_op[conv_op_channels][conv_op_size][conv_op_size]);
...
int main()
{
    fp ip_img [image_channels][image_size][image_size] = { {  { 1, 2, 3,0 }, {5,2,1,3}, {0,4,2,1},{0,0,0,0} }, { {0,2,5,3}, {9,0,6,1}, {7,5,2,5},{0,0,0,0} } , { {3,2,0,1}, {7,2,3,5},{0,6,2,1},{0,0,0,0}} };
	fp conv_kernels [kernel_filters][kernel_channels][kernel_size][kernel_size] ={ { {  { 0, 1, 5 }, {2, 1, 0}, {3, 5, 9} }, { {7, 6, 0}, {2, 0, 3}, {4, 2, 1} } , { {3, 2, 1}, {0, 6, 7}, {5, 6, 0} } }, { {  { 3 , 7, 1 }, {9, 0, 2}, {5, 6, 3} }, { {5, 2, 1}, {3, 6, 0}, {2, 1, 4} } , { {0, 5, 1}, {3, 7, 4}, {6, 2, 0} } } };
	fp kernel_bias [kernel_biases]= {1,2};
	fp res[conv_op_channels][conv_op_size][conv_op_size];
